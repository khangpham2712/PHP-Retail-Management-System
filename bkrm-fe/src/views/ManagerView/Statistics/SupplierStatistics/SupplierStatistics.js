...
const SupplierStatistics = () => {
  ...
  useEffect(() => {
    ...
    var dataRevenue= {   
      series: [{
        name: 'Tổng tiền nhập',
        data: topSortedRevenue ? topSortedRevenue.map((item) =>item.total_purchase_price).slice(0, limit) :[],
      }],
      options: {
        ...data.options,
        xaxis:{
  ...
  return (
    ...
    typeTitle={getTitle()}
    limitTitle={`Top ${limit} nhà cung cấp `}
     ToolBar={
       <>
...
const title = [" số lần nhập", " tổng tiền nhập"]
...
const DetailStatistic= (props) =>{
  ...
  const handleChangeType = (e)  =>{
    ...
    if(e.target.value.includes("best-seller")){
      newProductData.sort((a, b) => Number(b.total_purchase_price) - Number(a.total_purchase_price) )
  ...
  return(
    <ReportCard  title={`Báo cáo chi tiết`}  
        ToolBar={
          ...
          <TextField  style={{  marginLeft: 10 }}  variant="outlined"    placeholder={"Tìm nhà cung cấp..."} 
              InputProps={{ 
        ...
        <Grid container justifyContent='space-between' alignItems='center'  >
            <Grid item xs={4}><Typography style={{fontSize:17,fontWeight:500,}}>Nhà cung cấp</Typography></Grid>
            <Grid item xs={2}><Typography style={{ fontSize:17,fontWeight:500,textAlign:"center"}}>Số lần nhập</Typography></Grid>
            <Grid item xs={2}><Typography style={{fontSize:17,fontWeight:500,textAlign:"center"}}>Tổng tiền nhập</Typography></Grid>
