...
import InventorySidePanelDetail from "../../../components/CheckoutComponent/CheckoutSummary/ImportSummary/InventorySidePanelDetail";
import { ImportTableRowInventoryDetail } from "./ImportTableRow/ImportTableRowInventoryDetail";
...
const TransferDetail = (props) => {
  ...
  useEffect(() => {
    ...
    if (transferInventory) {
      ...
      const newData = transferInventoryData.transfer_detail.map((item) => {
        return {
          ...item,
              ...
              {!mode ? (
                <TableWrapper isCart={true}>
                  <TableHeader
                    ...
                    headerData={HeadCells.TableInventoryHeadCellsDetail}
                    isCart={true}
                  />
                  <TableBody>
                    {(dataCart || []).map((row, index) => {
                      return (
                        <ImportTableRowInventoryDetail
                          key={`${row.id}_index`}
                          row={row}
